...
void AShapesSubscriberManager::BeginPlay()
{
	...
	/* Find the topic */
	auto Squaretopic = dds::topic::find<dds::topic::Topic<dds::core::xtypes::DynamicData>>
		(participant, "Square");
	/* If the topic doesn’t exist create it */
	if (Squaretopic == dds::core::null)
	{
		Squaretopic = dds::topic::Topic<dds::core::xtypes::DynamicData>(participant,
			"Square", myType);
	}

	/* Create the topic with the configured name for the participant and dynamic type */
    /* Find the topic */
	auto Circletopic = dds::topic::find<dds::topic::Topic<dds::core::xtypes::DynamicData>>
		(participant, "Circle");
	/* If the topic doesn’t exist create it */
	if (Circletopic == dds::core::null)
	{
		Circletopic = dds::topic::Topic<dds::core::xtypes::DynamicData>(participant,
			"Circle", myType);
	}

	/* Create the topic with the configured name for the participant and dynamic type */
    /* Find the topic */
	auto Triangletopic = dds::topic::find<dds::topic::Topic<dds::core::xtypes::DynamicData>>
		(participant, "Triangle");
	/* If the topic doesn’t exist create it */
	if (Triangletopic == dds::core::null)
	{
		Triangletopic = dds::topic::Topic<dds::core::xtypes::DynamicData>(participant,
			"Triangle", myType);
	}
	...
	reader_count = dds::sub::find<dds::sub::DataReader<dds::core::xtypes::DynamicData> >(
		subscriber,
		"Circle",
		std::back_inserter(readers));
...
void AShapesSubscriberManager::Tick(float DeltaTime)
{
	...
	if (SquareReader != dds::core::null)
	{
		...
		rti::sub::LoanedSamples<dds::core::xtypes::DynamicData> samples =
			SquareReader.select().state(dds::sub::status::DataState::new_instance()).read();
		...
		for (const auto& sample : samples)
		{
			if (sample->info().valid())
			{
				...
				{
					/* Set the color of the new instance and spawn the subscriber */
	...
	if (CircleReader != dds::core::null)
	{
		...
		rti::sub::LoanedSamples<dds::core::xtypes::DynamicData> samples =
			CircleReader.select().state(dds::sub::status::DataState::new_instance()).read();
	...
	if (TriangleReader != dds::core::null)
	{
		...
		rti::sub::LoanedSamples<dds::core::xtypes::DynamicData> samples =
			TriangleReader.select().state(dds::sub::status::DataState::new_instance()).read();
